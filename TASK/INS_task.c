...
#define BMI088_SERTEMP 30  //陀螺仪控制温度
/**
...
void INS_task(void const *pvParameters)
{
    ...
    while((error_init=BMI088_init())!=0)
    {
        osDelay(100);
    }
...
static void imu_temp_control(fp32 temp)
{
    ...
    if (first_temperate)
    {
			  PID_Calculate(&PID_HEAT_PWM,temp,BMI088_SERTEMP);
        if (PID_HEAT_PWM.pos_out < 0.0f)
        {
            PID_HEAT_PWM.pos_out = 0.0f;
        }
        tempPWM = (uint16_t)PID_HEAT_PWM.pos_out;
    ...
    else
    {
        ...
        if (temp > BMI088_SERTEMP)
        {
            ...
            if (temp_constant_time > 200)
            {
                ...
                //
                first_temperate = 1;
                PID_HEAT_PWM.iout = MPU6500_TEMP_PWM_MAX / 2.0f;
            }
        }
