...
void shoot_task(void const *pvParameters)
{
    ...
    shoot_speed=4500;//摩擦轮速度
    dial_speed_set=2000;//拨弹速度4350
    PID_Init(&PID_M2006[0],POSITION_PID,16000,12000,10,0,0);		// 发射左拨盘电机 速度闭环
    for(int i=4; i<6; i++)	// 两个摩擦轮电机
    {
        PID_Init(&PID_M3508[i],POSITION_PID,16000,16000,5,0,0.1);// 0.01//5 0.1 0.1
    }	
...
void shoot_pc_ctrl(void){
	...
	if((rc.key[6] || rc.mouse_press_l==1) &&  shoot_flag.friction==0)//首次按q打开摩擦轮
		shoot_flag.friction=1;
	else if(rc.key[7] && shoot_flag.friction==1)//再按q关闭摩擦轮
		shoot_flag.friction=0;
  if(rc.mouse_press_l==1 && shoot_flag.shoot_continuous==0 && abs(CAN_M3508[4].speed)>500){
...
void shoot_rc_ctrl(void) {
    ...
    if(rc.sw1==2 && shoot_flag.shoot_continuous==0 && abs(CAN_M3508[4].speed)>500){
			shoot_flag.shoot_continuous=1;
	  }
	  if(rc.sw1==3) {
...
//		shoot_flag.friction = 0;//热量限制
	
		if(magazine_flag)__HAL_TIM_SetCompare(&htim1, TIM_CHANNEL_1, 1700);
	  else  __HAL_TIM_SetCompare(&htim1, TIM_CHANNEL_1, 590);//弹仓盖标志
