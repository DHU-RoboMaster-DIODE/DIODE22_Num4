...
CAN_M3508_TypeDef 	CAN_M3508[7]	=	{0};	// 前四个是底盘电机 最后两个是摩擦轮电机
...
float C_yaw,last_C_yaw,total_C_yaw=3000;
...
{	
    ...
    tx_data[2] = 0;
    tx_data[3] = 0;
    tx_data[4] = (CAN_GM6020[1].set_voltage >> 8)&0xff;
    tx_data[5] = (CAN_GM6020[1].set_voltage)&0xff;
    tx_data[6] = 0;
    tx_data[7] = 0;
    ...
    tx_data[4] = 0;
    tx_data[5] = 0;
    tx_data[6] = (CAN_M3508[6].set_current >> 8)&0xff;
    tx_data[7] = (CAN_M3508[6].set_current)&0xff;
    CAN_SendMsg(&hcan2,0,0x200,8,tx_data);
}
...
void CAN_Shoot_SendCurrent(void)
{
    ...
    data[2] = CAN_M3508[4].set_current >> 8;
    data[3] = CAN_M3508[4].set_current;
    data[4] = CAN_M3508[5].set_current >> 8;
    data[5] = CAN_M3508[5].set_current;
    data[6] = 0;
    data[7] = 0;
...
//						else if(RxMeg.StdId == 0x212){
//								C_yaw=(float)((rx_data[0]<<8)+rx_data[1])/10.0;
//							
//						}
    ...
    if(hcan->Instance == CAN2)
    {
        ...
        if ( HAL_OK==HAL_RetVal)
        {
            ...
            /* GM6020的反馈信息处理 */
            if (RxMeg.StdId == CAN2_GM6020_ID3)	/* 0x209 */
            {
                CAN2_CNT ++;
                ...
                CAN_GM6020[1].temperature  =  rx_data[6];
			      ...
			      else if(RxMeg.StdId >= CAN2_M3508_ID2 & RxMeg.StdId <= CAN2_M3508_ID4)	/* 0x202 ~ 0x203*/
            {
                CAN2_CNT ++;
                id = RxMeg.StdId-CAN2_M3508_ID2+4;
                // 将数据保存在结构体中
                CAN_M3508[id].angle=(rx_data[0]<<8)+rx_data[1];
                CAN_M3508[id].speed=(rx_data[2]<<8)+rx_data[3];
                CAN_M3508[id].current=(rx_data[4]<<8)+rx_data[5];
                CAN_M3508[id].temperature=(rx_data[6]<<8);
						...
						else if(RxMeg.StdId == CAN2_M2006_ID1)	
            {
                CAN2_CNT ++;
                // 将数据保存在结构体中
...
//				        get_total_angle_2006(&CAN_M2006[0]);
            }
            if (CAN2_CNT == 500)
            {
                CAN2_CNT = 0;
